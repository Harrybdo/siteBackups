...
<article id="post-<?php the_ID(); ?>" <?php post_class('clearfix'); ?> role="article" itemscope itemtype="http://schema.org/BlogPosting">
    ...
    <img class="alignnone size-full wp-image-311 banner-image" src="<?php echo get_template_directory_uri()?>/library/images/blog/<?php echo $blog_banner_image; ?>" alt="blog banner image" />
		...
		<?php get_template_part( 'partials/content', 'byline-single' ); ?>
	...
	<footer class="article-footer">
		...
		<h4 class="blog-specialty-list-header">Associated Services:</h4>
    <ul class="blog-specialty-list">
    <?php foreach($services as $service){
      $service_item = $service['service'];
      $service_item_href = $service['service_href'];
    ?> 
